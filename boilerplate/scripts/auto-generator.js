...
`;
    ...
    // Update common-types.ts file to use correct private state type
    await this.updateCommonTypes(contractInfo);
    
    console.log('✅ Core API updated');
  }

  /**
   * Update common-types.ts to use the correct private state type
   */
  async updateCommonTypes(contractInfo) {
    const commonTypesPath = path.join(this.config.cliSourceDir, 'src', 'common-types.ts');
    
    try {
      // Detect the actual private state type from witnesses.ts
      const privateStateType = await this.detectPrivateStateTypeFromWitnesses();
      const contractBaseType = privateStateType.replace('Private', '').replace('State', '');
      
      // Generate the complete common-types.ts file dynamically
      const commonTypesContent = `import { contracts } from '@midnight-ntwrk/contract';
import type { ${privateStateType} } from '@midnight-ntwrk/contract';
...
export type { ${privateStateType} };
export type ${contractBaseType}Circuits = ImpureCircuitId<typeof contractModule.Contract>;

export const ${contractBaseType}PrivateStateId = '${contractBaseType.toLowerCase()}PrivateState';

export type ${contractBaseType}Providers = MidnightProviders<${contractBaseType}Circuits, typeof ${contractBaseType}PrivateStateId, ${privateStateType}>;

export type ${contractBaseType}Contract = typeof contractModule.Contract;

export type Deployed${contractBaseType}Contract = DeployedContract<${contractBaseType}Contract> | FoundContract<${contractBaseType}Contract>;
`;

      await fs.promises.writeFile(commonTypesPath, commonTypesContent, 'utf8');
      console.log(`🔧 Generated common-types.ts with ${privateStateType}`);
      
      // Now update all CLI source files to use the correct contract-specific types
      await this.updateAllCLITypes(contractInfo, privateStateType, contractBaseType);
      
      // Update deployment code to match the private state structure
      // await this.updateDeploymentCode(privateStateType, contractBaseType);
    } catch (error) {
      console.warn('⚠️  Could not update common-types.ts:', error.message);
    }
  }

  /**
   * Detect the actual private state type from witnesses.ts file
   */
  async detectPrivateStateTypeFromWitnesses() {
    const witnessesPath = path.join(this.config.contractSourceDir, 'witnesses.ts');
    
    if (!fs.existsSync(witnessesPath)) {
      throw new Error('witnesses.ts file not found');
    }

    const witnessContent = fs.readFileSync(witnessesPath, 'utf-8');
    
    // Look for export type declarations like "export type BBoardPrivateState" or "export type ContractPrivate"
    const exportTypeMatch = witnessContent.match(/export type (\w+(?:Private|PrivateState))/);
    if (exportTypeMatch) {
      return exportTypeMatch[1];
    }

    // Look for type declarations like "type BBoardPrivateState"
    const typeMatch = witnessContent.match(/type (\w+(?:Private|PrivateState))/);
    if (typeMatch) {
      return typeMatch[1];
    }

    // Fallback: look for any type that ends with "Private" or "PrivateState"
    const privateTypeMatch = witnessContent.match(/(\w+(?:Private(?:State)?)) = \{/);
    if (privateTypeMatch) {
      return privateTypeMatch[1];
    }

    throw new Error('Could not detect private state type from witnesses.ts');
  }

  /**
   * Update all CLI source files to replace Counter types with contract-specific types
   */
  async updateAllCLITypes(contractInfo, privateStateType, contractBaseType) {
    // Define the type mappings from Counter* to contract-specific types
    const typeMapping = {
      'CounterContract': `${contractBaseType}Contract`,
      'CounterPrivateState': privateStateType,
      'CounterPrivateStateId': `${contractBaseType}PrivateStateId`,
      'CounterProviders': `${contractBaseType}Providers`,
      'DeployedCounterContract': `Deployed${contractBaseType}Contract`,
      'CounterCircuits': `${contractBaseType}Circuits`,
      // Also handle existing incorrect types that might be in the files
      'BBoardContract': `${contractBaseType}Contract`,
      'BBoardPrivateState': privateStateType,
      'BBoardPrivateStateId': `${contractBaseType}PrivateStateId`,
      'BBoardProviders': `${contractBaseType}Providers`,
      'DeployedBBoardContract': `Deployed${contractBaseType}Contract`,
      'BBoardCircuits': `${contractBaseType}Circuits`,
      'BboardContract': `${contractBaseType}Contract`,
      'BboardPrivateState': privateStateType,
      'BboardPrivateStateId': `${contractBaseType}PrivateStateId`,
      'BboardProviders': `${contractBaseType}Providers`,
    ...
    // Files to update in the CLI src directory
    const filesToUpdate = [
      'api.ts',
      'cli.ts', 
      'dynamic-cli-generator.ts',
      'simple-enhanced-cli.ts'
    ];

    for (const fileName of filesToUpdate) {
      const filePath = path.join(this.config.cliSourceDir, 'src', fileName);
      ...
      try {
        ...
        // Replace type imports and usages
        for (const [oldType, newType] of Object.entries(typeMapping)) {
          // Replace in import statements
          const importRegex = new RegExp(`(import\\s*\\{[^}]*?)\\b${oldType}\\b([^}]*\\})`, 'g');
          if (content.match(importRegex)) {
            content = content.replace(importRegex, `$1${newType}$2`);
            modified = true;
          }

          // Replace type usage throughout the file
          const typeUsageRegex = new RegExp(`\\b${oldType}\\b`, 'g');
          if (content.match(typeUsageRegex)) {
            content = content.replace(typeUsageRegex, newType);
            modified = true;
          }
        }
        ...
        if (modified) {
          ...
          console.log(`🔧 Updated types in ${fileName} to use ${contractBaseType} types`);
        }
      } catch (error) {
  ...
  /**
   * Update CLI deployment code to use the correct private state structure
   */
  async updateDeploymentCode(privateStateType, contractBaseType) {
    const filesToUpdate = ['cli.ts', 'simple-enhanced-cli.ts'];
    
    // Analyze the private state structure
    const witnessesPath = path.join(this.config.contractSourceDir, 'witnesses.ts');
    const witnessContent = fs.readFileSync(witnessesPath, 'utf-8');
    
    // Extract the private state fields
    const privateStateMatch = witnessContent.match(/type\s+\w+\s*=\s*\{([^}]+)\}/s);
    if (!privateStateMatch) return;
    
    const fieldsText = privateStateMatch[1];
    const hasSecretKey = fieldsText.includes('secretKey');
    const hasPrivateCounter = fieldsText.includes('privateCounter');
    
    let deploymentObj;
    if (hasSecretKey && !hasPrivateCounter) {
      deploymentObj = '{ secretKey }';
    } else if (hasPrivateCounter && !hasSecretKey) {
      deploymentObj = '{ privateCounter: 0 }';
    } else {
      // Generic fallback
      const fields = fieldsText.match(/readonly\s+(\w+):/g);
      if (fields && fields.length > 0) {
        const fieldName = fields[0].replace('readonly ', '').replace(':', '').trim();
        if (fieldName.includes('secret') || fieldName.includes('key')) {
          deploymentObj = `{ ${fieldName}: secretKey }`;
        } else {
          deploymentObj = `{ ${fieldName}: 0 }`;
        }
      } else {
        deploymentObj = '{}';
      }
    ...
    for (const fileName of filesToUpdate) {
      ...
      try {
        ...
        // Only replace if we have incorrect privateCounter usage
        if (content.includes('{ privateCounter: 0 }') && hasSecretKey) {
          content = content.replace(/\{\s*privateCounter:\s*0\s*\}/g, deploymentObj);
          modified = true;
          
          // Ensure crypto import is present
          if (deploymentObj.includes('secretKey') && !content.includes('crypto')) {
            // Add crypto import after the first few imports
            const importPattern = /(import[^;]+from\s+['"][^'"]+['"];\s*\n){2,3}/;
            const match = content.match(importPattern);
            if (match) {
              const insertPoint = content.indexOf(match[0]) + match[0].length;
              const cryptoImport = "import { webcrypto as crypto } from 'node:crypto';\n";
              content = content.slice(0, insertPoint) + cryptoImport + content.slice(insertPoint);
              modified = true;
            }
          }

          // Add secret key generation before deployment calls
          if (deploymentObj.includes('secretKey')) {
            const deployPattern = /api\.deploy\(providers,\s*\{\s*secretKey\s*\}\)/g;
            content = content.replace(deployPattern, (match) => {
              return `// Generate a random 32-byte secret key
      ...
      api.deploy(providers, { secretKey })`;
        ...
        if (modified) {
          await fs.promises.writeFile(filePath, content, 'utf8');
          console.log(`🔧 Updated deployment code in ${fileName}`);
        }
      } catch (error) {
        console.warn(`⚠️  Could not update deployment code in ${fileName}:`, error.message);
      }
    }
  }
